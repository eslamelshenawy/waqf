            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <div class="card-title"><br><h4>{{ __('admin::privileges._the_permission', ['something' => __('shared::actions.edit')]) }}</h4><br></div>
                        <hr>
                        <form class="needs-validation" novalidate method="POST" action="{{ route('Admin::permissions.update', $permission->id) }}">
                            @csrf
                            @method('PATCH')
                            <div class="form-row">
                                <div class="col-md-6 mb-3">
                                    <label for="name">
                                        {{ __('admin::privileges._the_permission', ['something' => __('admin::privileges.name')]) }}
                                    </label>
                                    <input type="text" class="field form-control @error('name') border border-danger @enderror"
                                           name="name" autocomplete="off" id="name" value="{{ old('name', $permission->name) }}"
