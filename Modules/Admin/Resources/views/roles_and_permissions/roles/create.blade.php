                                    ...
                                    <div class="form-row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input required="required" type="text" class="field form-control @error('name') border border-danger @enderror" name="name"
                                                   autocomplete="off" id="roleName1" value="{{ old('role_name') }}" placeholder="{{ __('admin::privileges._role', ['something' => __('admin::privileges.name')]) }}">
                                            @error('name')
                                            <div>
                                        ...
                                        <div class="col-md-12">
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-hover">
                                                    ...
                                                    <form method="POST" action="{{ route('Admin::roles.store') }}" name="create_role" id="createRole">
                                                        <input type="hidden" name="name" id="roleName2">
                                                    @foreach(getModels() as $model)
                                                        <tr>
                                                            @csrf
                                                            <td>{{ $model = str_replace('.php', '', $model) }}</td>
                                                            <td><input type="checkbox" class="checkbox" name="create[]" value="{{ $model }}"></td>
                                                            <td><input type="checkbox" class="checkbox" name="read[]" value="{{ $model }}"></td>
                                                            <td><input type="checkbox" class="checkbox" name="update[]" value="{{ $model }}"></td>
                                                            <td><input type="checkbox" class="checkbox" name="delete[]" value="{{ $model }}"></td>
                                                        </tr>
                                                    @endforeach
                                                    </form>
                                                </table>
