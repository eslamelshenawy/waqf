    ...
    <div class="form-row">
        ...
        <div class="col-md-3 mb-1">
            <input type="radio" name="services[]" value="electric"
                   {{ old('services', $agency->services) == 'electric' ? 'checked' : '' }}
        ...
        <div class="col-md-3 mb-1">
            <input type="radio" name="services[]"
                   ...
                   {{ old('services', $agency->services) == 'transfer_furniture' ? 'checked' : '' }}
            class="form-check-inline" onchange="chooseService(this)" />
            <label>&nbsp;{{ __('shared::users.transfer_furniture') }}</label>
        </div>
        <div class="col-md-3 mb-1">
            <input type="radio" name="services[]" value="uploading_and_downloading"
                   {{ old('services', $agency->services) == 'uploading_and_downloading' ? 'checked' : '' }}
                   class="form-check-inline" onchange="chooseService(this)" />
            <label>&nbsp;{{ __('shared::users.uploading_and_downloading') }}</label>
        </div>
        <div class="col-md-3 mb-1">
            <input type="radio" name="services[]" value="cleaning"
                   {{ old('services', $agency->services) == 'cleaning' ? 'checked' : '' }}
                   class="form-check-inline" onchange="chooseService(this)" />
            <label>&nbsp;{{ __('shared::users.cleaning') }}</label>
        </div>
        <div class="col-md-12 mb-1">
            <input type="radio" name="services[]" value="other" {{ old('services', $agency->services) == 'other' ? 'checked' : ''  }}
                   class="form-check-inline"
                   id="otherServiceRadio" onchange="chooseService(this)" />
            <label>&nbsp;{{ __('shared::commons.other') }}</label>
        </div><br>
        <div class="col-md-3 mb-1" style="display: {{ $agency->services == 'other' ? 'block' : 'none' }}" id="otherServiceDiv">
            <input type="text" name="service_other" value="{{ old('service_other', $agency->service_other) }}"
                   class="form-control"
