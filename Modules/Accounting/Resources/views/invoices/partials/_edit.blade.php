...
{{--        </button>--}}
    ...
    <form method="POST" action="{{ route('Accounting::invoices.store') }}" id="invoiceForm" name="invoice_form">
        @csrf
        ...
        <div class="row justify-content-between">
            <div class="col-md-10">
                ...
                <div class="col-sm-4 form-group mb-3 pl-0">
                    ...
                    <input disabled type="text" name="order_number" value="{{ ++$invoiceCount }}"  class="form-control @error('order_number') border border-danger @enderror" id="invoiceNumber"
                ...
                <div class="col-sm-4 form-group mb-3 pl-0">
                    ...
                    <input type="text" name="invoice_code" value="{{ old('invoice_code') }}" maxlength="8" class="form-control @error('invoice_code') border border-danger @enderror" id="invoiceCode"
                ...
                <div class="col-sm-4 form-group mb-3 pl-0">
                    <div class="form-group mb-3">
                        ...
                        <input id="order-datepicker" value="{{ old('order_at') }}" class="form-control text-right @error('order_at') @enderror" placeholder="yyyy-mm-dd" name="order_at">
                        @error('order_at')
                        <div>
                            <span class="badge badge-danger">{{ $message }}</span>
                        </div>
                ...
                <div class="col-sm-12 col-md-12 form-group mb-3 pl-0">
                    ...
                    <textarea class="form-control  @error('notes') border border-danger @enderror" name="notes" id="notes"
                              placeholder="{{ __('accounting::_.notes') }}" value="{{ old('notes') }}" rows="4" ></textarea>
                    @error('notes')
                    <div>
                        <span class="badge badge-danger">{{ $message }}</span>
                    </div>
...
{{--                        <input type="radio" name="document_type" {{ old('document_type')=="cash" ?  "checked" : ""}}class="form-control" value="cash" id="documentTypeCash" onchange="determineDocumentType(this.value)">--}}
{{--                        <span>{{ __('accounting::_.cash') }}</span>--}}
{{--                        <span class="checkmark"></span>--}}
{{--                    </label>--}}
{{--                    <label class="radio radio-reverse radio-warning">--}}
{{--                        <input type="radio" class="form-control @error('document_type') @enderror"{{ old('document_type')=="credit" ?  "checked" : ""}} name="document_type"  value="credit" id="documentTypeCredit" onchange="determineDocumentType(this.value)">--}}
...
{{--                <div class="pr-0 mb-4">--}}
{{--                    <label class="radio radio-reverse radio-danger">--}}
{{--                        <input type="radio" class="form-control" name="payment_mode" {{ old('payment_mode') =="fund" ?  "checked" : ""}} value="fund" id="paymentModeFund" onchange="determinePaymentMode(this.value)">--}}
{{--                        <span>{{ __('accounting::_.fund') }}</span>--}}
{{--                        <span class="checkmark"></span>--}}
{{--                    </label>--}}
{{--                    <label class="radio radio-reverse radio-warning">--}}
{{--                        <input type="radio" class="form-control @error('payment_mode') @enderror" name="payment_mode"{{ old('payment_mode')=="bank" ?  "checked" : ""}} value="bank" id="paymentModeBank" onchange="determinePaymentMode(this.value)">--}}
...
{{--            </div>--}}
                    ...
                    <div class="col-md-10 form-group mb-3 pl-0">
                        <select class="form-control @error('fund_id') border border-danger @enderror" name="fund_id">
                            ...
                            @foreach(\Modules\Accounting\Entities\Account::whereIn('id',[21,22])->get() as $fund)
                                <option value="{{ $fund->id }}" {{ old('fund_id')== $fund->id ?  "selected" : ""}}>{{ $fund->name }} && {{$fund->balance }}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('fund_id')
        ...
        <div class="row">
            <div class="col-md-6" id="tenantDiv" >
                ...
                <div class="col-md-10 offset-md-2 form-group mb-3 pr-0">
                    ...
                    <select class="form-control @error('tenant_id') border border-danger @enderror" name="tenant_id">
                        ...
                        @foreach(\App\Tenant::all() as $tenant)
                            <option value="{{ $tenant->id }}"{{ old('tenant_id',$tent)== $tenant->id ?  "selected" : ""}}>{{ $tenant->name }}</option>
                        @endforeach
                    </select>
                </div>
                @error('tenant_id')
                ...
                @enderror
                <div class="col-md-10 offset-md-2 form-group mb-3 pr-0">
                    <textarea class="form-control text-right" placeholder="Bill From Address" name="description_invoice" value="{{ old('description_invoice') }}"></textarea>
                </div>
            </div>
        </div>
        <div class="col-md-4 mb-4">
            ...
            <select class="form-control  @error('account_id') border border-danger @enderror" name="account_id" id="account_id" required>
                <option selected disabled>{{ __('shared::actions.select') }}</option>
                @foreach($account->get() as $c)
                    <option value="{{ $c->id }}" {{ old('account_id') == $c->id ? 'selected' : '' }} >{{ $c->name  }}
                    </option>
                @endforeach
            </select>
            @error('account_id')
...
{{--                        <th scope="col">{{ __('accounting::_.service_', ['something' => 'وصف']) }}</th>--}}
                    ...
                    <tr>
                        <th scope="row">1</th>
                        <td>
                            <input name="service_name" type="text" class="form-control @error('service_name') border border-danger @enderror" value="{{ old('service_name') }}"
                        ...
                        <td>
                            <input name="service_price" type="text" maxlength="5" class="form-control @error('service_price') border border-danger @enderror" value="{{ old('service_price',$price) }}"
                    ...
                    <table class="table table-borderless">
                        <tr>
                            ...
                            <td colspan="2"><input type="text" id="amount" name="amount" disabled
                                                   class="form-control small-input mb-1 @error('amount') border border-danger @enderror" value="{{ old('service_price',$price)  }}"></td>
                        </tr>
{{--                        <tr>--}}
{{--                            <td>{{ __('accounting::_.paid_amount') }}</td>--}}
{{--                            <td><input type="text" id="paid_amount" name="paid_amount"--}}
{{--                                       class="form-control small-input mb-1 @error('paid_amount') border border-danger @enderror" value="{{ old('paid_amount') ? old('paid_amount') : 0 }}"--}}
...
{{--                                       class="form-control small-input mb-1 @error('remaining_amount') border border-danger @enderror" value="{{ old('remaining_amount') ? old('remaining_amount') : 0 }}" readonly></td>--}}
{{--                        </tr>--}}
                    </table>
